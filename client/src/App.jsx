...
function App() {
  ...
  const loadMonthStats = async () => {
    try {
      ...
      const res = await fetch(
        `${API.CALENDAR}/month-stats?year=${year}&month=${month}`,
      );
      setMonthStats(await res.json());
    } catch (error) {
      console.error('Помилка завантаження статистики місяця:', error);
    }
  };
  ...
  const handleDayClick = (day) => {
    ...
    const localISODate = new Date(day.getTime() - tzOffset)
      .toISOString()
      .split('T')[0];
  ...
  const handleDeletedAppointment = async (id) => {
    ...
    try {
      ...
      if (res.ok) loadData(selectedDate);
      loadMonthStats();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDeleteClient = async (id) => {
    if (
      !window.confirm(
        'Видалити цю клієнтку? Увага: всі її візити та історія також будуть видалені!',
      )
    )
      return;
    try {
  ...
  const handleSendReminder = (slot) => {
    const formattedDate = new Date(selectedDate).toLocaleDateString('uk-UA', {
      day: 'numeric',
      month: 'short',
    });
    ...
    if (slot.clientPhone) {
      window.open(
        `viber://chat?number=${encodeURIComponent(slot.clientPhone)}&draft=${encodedText}`,
        '_blank',
      );
      console.log(encodedText);
      setTimeout(() => {
        window.open(
          `https://viber.click{cleanPhone}?text=${encodedText}`,
          '_blank',
        );
      }, 300);
    } else {
  ...
  const handleUpdateClientNotes = async (clientId, newNotes) => {
    try {
      setClients((prev) =>
        prev.map((c) => (c._id === clientId ? { ...c, notes: newNotes } : c)),
      );
      ...
      console.error('Не вдалося зберегти нотатки:', error);
    }
  };
        ...
        <div className="max-w-md mx-auto px-4 pt-4 space-y-4">
          ...
          <Routes>
            ...
            <Route
              ...
              element={
                <ClientsPage
                  ...
                  handleUpdateClientNotes={handleUpdateClientNotes}
            ...
            <Route
              ...
              element={
                <PricePage
                  services={services}
                  setIsNewServicePopupOpen={setIsNewServicePopupOpen}
                ...
                <div className="space-y-1 overflow-y-auto flex-1 pr-1 bg-slate-50/50 p-2 rounded-xl border border-slate-100">
                  {daySchedule.map((slot, index) => {
                    ...
                    if (slot.isBusy && slot.id) {
                      const firstSlotIndex = daySchedule.findIndex(
                        (t) => t.id === slot.id,
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span className="font-bold px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-500">
                              {slot.time}
                            </span>
                            <span className="italic text-[11px] text-slate-300">
                              Вільна година (записати)
                            </span>
                          </div>
                        </div>
                      );
                    ...
                    const clientSlots = daySchedule.filter(
                      (t) => t.id === slot.id,
                    );
                    const lastSlotTime =
                      clientSlots[clientSlots.length - 1].time;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-slate-700 bg-white/80 px-2 py-0.5 rounded-md text-[11px]">
                            {slot.price} ₴
                          </span>
                          <button
                            type="button"
                            onClick={() => handleDeletedAppointment(slot.id)}
                ...
                <form onSubmit={handleAddAppointment} className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    {clientPhone.length > 0 &&
                      clients.filter((c) => c.phone.includes(clientPhone))
                        .length > 0 &&
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        type="date"
                        value={appointmentDate || selectedDate} // Використовує поточну обрану датуcalend
                        onChange={(e) => {
                          const newDate = e.target.value;
                          setSelectedDate(newDate); // Записуємо дату форми
                          loadData(newDate); // Оновлюємо години для цього дня
                        }}
                        required
                      ...
                      >
                        ...
                        {Array.from({ length: 25 }).map((_, i) => {
                          ...
                          const isTimeBusy =
                            matchingSlot &&
                            matchingSlot.isBusy &&
                            matchingSlot.id !== editingAppointmentId;
                          return (
                            <option
                              key={tStr}
                              value={tStr}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-pink-600 focus:outline-hidden focus:border-pink-500 focus:bg-white"
                    />
                  </div>
                  <button
                    type="submit"
                    className="w-full bg-pink-600 hover:bg-pink-700 text-white text-sm font-bold py-2.5 rounded-xl cursor-pointer shadow-xs shadow-pink-100"
                  >
                    Зберегти запис
                  </button>
                </form>
              ...
              <div className="bg-white w-full max-w-sm rounded-2xl p-5 shadow-xl space-y-4">
                <div className="flex justify-between items-center">
                  <h2 className="text-md font-extrabold text-slate-800">
                    🛠 Додати послугу в прайс
                  </h2>
                  <button
                    type="button"
                    onClick={() => setIsNewServicePopupOpen(false)}
                ...
                <form onSubmit={handleAddService} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-slate-800 hover:bg-slate-900 text-white text-sm font-bold py-2.5 rounded-xl cursor-pointer transition-all"
                  >
                    Зберегти в прайс
                  </button>
                </form>
          ...
          <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200/80 px-4 py-2.5 flex justify-around items-center z-40 shadow-xl shadow-slate-900/10">
            ...
            <NavLink
              ...
              className={({
                isActive,
              }) => `flex flex-col items-center gap-0.5 text-[11px] font-bold transition-all cursor-pointer 
